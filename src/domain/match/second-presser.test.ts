...
const createTestMatch = (seed = 7) => createMatchState(buildMatchConfig(createDefaultProfile(), seed));
...
describe("segundo engajador na zona de perigo (Item 1)", () => {
  it("manda um zagueiro sair da linha quando a bola entra no terço defensivo sem pressão", () => {
    ...
    const presser = state.players.find((p) => p.team === "blue" && p.profile.position === "forward")!;
