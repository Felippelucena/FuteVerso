...
const createTestMatch = (seed = 9) => createMatchState(buildMatchConfig(createDefaultProfile(), seed));
...
describe("zagueiro-ofensivo moderado + recomposição (Item 4)", () => {
  it("libera o lateral a sobrepor no ataque; nenhum outro jogador recebe overlapRun", () => {
    ...
    fb.profile.position = "fullBack"; // vira lateral (mantém role "defender")
    fb.profile.skills.defending = 40; // não é o melhor candidato a segurança
    const mid = state.players.find((p) => p.team === "blue" && p.profile.position === "midfielder")!;
    mid.profile.skills.defending = 99; // será o jogador de segurança (rest defense)
    const fwd = state.players.find((p) => p.team === "blue" && p.profile.position === "forward")!;
  ...
  it("zagueiro adiantado recompõe em disparada garantida ao perder a posse", () => {
    ...
    const closeBlue = state.players.find((p) => p.team === "blue" && p.profile.position === "midfielder")!;
