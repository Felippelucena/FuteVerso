...
- runtime/pass-viability responde a **uma** pergunta, para o motor inteiro: *este passe chega?* Devolve a chance de conclusão como o produto das três coisas que precisam dar certo — a bola atravessa a rota, o recebedor vence a corrida, e ele domina. É consumida pelas duas pontas: `possession-system` a chama com a bola REAL que chegou (`receptionMargin` + o ruído do lance), e `decision/pass` com a bola PREVISTA que pensa em jogar. Antes a mesma pergunta tinha três respostas que não se conheciam — a decisão descontava a pressão da bola aérea em dois terços, a execução cobrava por rótulo e a recepção cobrava de novo —, e o motor escolhia 88% das vezes exatamente o passe que não conseguia completar. A nota do passe é hoje **valor × probabilidade**, não valor menos risco: é o que impede um valor grande de engolir qualquer risco;
- runtime/pass-trajectory resolve a altura do passe pela **geometria do corredor**, não pela duração: o voo é o maior entre o que basta para transpor o corpo que está na rota e o que a força da perna (`PHYSICS.aerialPassLaunchSpeed`) leva para cobrir a distância. Daí a bola curta sair rasante e a longa em parábola, sem caso especial — e daí sair também `reachableFlightSeconds`, quanto do voo a bola fica ao alcance de alguém, que é o prazo real para dominá-la;
